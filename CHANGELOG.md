...
- **改进**:重做无会话工作台首页,统一深色层级、卡片布局、侧栏品牌区和左下角设置入口
- **改进**:顶栏改为纯窗口拖拽区,移除冗余品牌与操作内容
- **修复**:会话或同一连接内终端标签切换回来后,强制重新计算 xterm 尺寸并刷新画布,避免终端偶发黑屏
- **工程化**:增加终端激活重排的回归测试

## [1.3.2] - 2026-08-26

- **修复**:同一 SSH 会话开启多个终端后，关闭子终端不再依赖远端 channel 异步 `close` 事件；关闭后立即移除对应标签，并避免重复状态通知
...
- **工程化**:CI 生产依赖审计提升为阻止 high/critical 漏洞
